...
public interface NetworkFetcher<FETCH_STATE extends FetchState> {
    ...
    /**
     * Initiates the network fetch and informs the producer when a response is received via（经过） the
     * provided callback.
     * <p>
     * 初始化网络的fetch,并注册回调
     *
    ...
    /**
     * 是否立即将事件传递出去
     * Gets whether the intermediate results should be propagated（传递）.
     * <p>
